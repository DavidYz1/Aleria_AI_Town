...
**一条可演示闭环：**玩家在地图上与 NPC 对话，发言以「玩家的说法」写入带来源的记忆；世界推进时，NPC 检索获准使用的记忆，生成或复用多步计划；每一步经 `ActionRegistry` 校验后才改变世界，并把目标、计划来源、规划时检索的可公开记忆和执行结果呈现在界面。模型不可用时，确定性策略接管。当前能力与限制见 [阶段快照](docs/eval/2026-09-16-stage3m-snapshot.md)，复现数据见 [当前 Fake 评测](docs/eval/2026-09-16-agent-eval-fake.md)。**Fake 数字只证明链路可复现，真实模型质量仍需另测。**

项目还有一条独立的 [AI 辅助开发 workflow](#ai-开发工具与人工修正案例)：由人决定目标和边界，AI 辅助设计与编码，经过 TDD、测试、评测和人工 review 后由人提交。它是开发方法，不是 NPC 的 Agent 运行链路。
...
这个匿名只读区域使用本地确定性 query embedding，不调用配置的远端 Embedding Provider；内部检索在 Embedding 不可用时可降级为词面检索。地图、详情、对话、推进和任务仍可用。
...
[Identity]   角色与性格
[Needs]      体力 / 心情 / 社交 + 阈值提示
[World]      当前位置、**本职岗位地点**、同场 NPC、可达地点
[Episodic]   检索出的相关经历（有预算上限）
[Semantic]   当前实现为占位文案；尚未把 Belief 行接入规划上下文
[Procedural] 当前计划与进度
[Tools]      注册表中的动作名（完整定义随 tool calling 参数发送）
[LastOutcome] 当前为「本次决策的起点」占位，尚未回传上一步结果
```

**工具契约必须完整表达引擎强制的前置条件。** `work` 只能在本职岗位执行、`eat` 只能在酒馆执行——这两条在 `ActionRegistry` 里强制，但工具描述一度只写「不需要目标」，`[World]` 段也从没告诉 NPC 自己的岗位在哪。模型在任意地点调用它们是完全合理的推断，引擎却必拒：**被惩罚的是它无从知晓的规则**。这类问题看起来像「模型不够聪明」，实际是接口没说清楚。现在两条前置条件都写进了工具描述，岗位地点也在 `[World]` 段单独点名。
...
- **跨 tick 反馈目前只完成一部分**：行动事件可经感知写入记忆；`[LastOutcome]` 段目前由 Service 固定传 `None`，尚未把上一步的校验结果直接回传给规划器。
...
`scripts/eval_agent.py` 在隔离的临时 SQLite 里跑 N 个 tick，记录 Provider 调用并输出 markdown 报告；不会读取或写入 `backend/data/aleria.db`。Fake 模式禁用 `.env` 与真实模型凭据。

```bash
python scripts/eval_agent.py --ticks 20 --provider fake --out docs/eval/2026-09-16-agent-eval-fake.md
# 仅在明确确认额度和耗时后执行：
python scripts/eval_agent.py --ticks 20 --provider live --out docs/eval/live-local.md
```

当前 HEAD 的 [20 tick Fake 报告](docs/eval/2026-09-16-agent-eval-fake.md)记录了 60 个 NPC-tick：计划复用 30/60，Provider 调用 30/60，动作未被规则替换 60/60，兜底 0/60。调用是本地替身，token 未上报，不能用这些数字推断真实模型质量或成本。报告同时给出 tick 与 Provider 耗时的 P50/P95、明确分母和数据来源；[去内容化的逐次证据](docs/eval/2026-09-16-agent-eval-fake.evidence.json)供核对。

兜底原因是**分层归因**的，不是一个笼统的百分比：

```
fallback
├── provider:*   adapter 按异常类型分 —— timeout / http_status / transport / parse_error
└── rule:*       ActionRegistry 与冲突处理的拒绝码 —— unknown_location / wrong_duty_location / …
```

`rule_rejection` trace 记录**被替换掉的原始提案**（动作、目标、来源）。这解决了一个具体问题：以前报告能说「3 次规则拒绝」，却答不出是哪条规则拒了什么动作——因为被拒的提案在进入校验前就被整体换成了兜底动作，原始内容和拒绝码一起丢了。

两次[真实模型探路](docs/eval/2026-09-17-agent-eval-live-probe-post-p0.md)（`hy3`，各 5 tick）验证了这条链路。归因落地后，8 次兜底**全部**报为 `provider:timeout`——对照 2026-09-14 报告里那句无法复核的「19 次超时」，同样的结论这次有 trace 支撑。

它还直接指出了下一步该改什么：超时上限配在 20 秒，而 8 次超时全部精确落在 20007–20024ms，3 次成功是 12622 / 18600 / 18521 ms——成功调用已经贴着上限。

于是[把上限调到 30 秒重测](docs/eval/2026-09-17-agent-eval-live-probe-timeout30.md)，拿到一条**不依赖对比**的证据：6 次成功调用里有 **3 次耗时超过 20 秒**（20113 / 20970 / 23959 ms），它们在旧上限下必然被判超时。兜底率从 53.3% 降到 6.7%。

还有一个容易被忽略的二阶效应：**提高超时反而减少了模型调用**（0.733 → 0.467 次/NPC-tick）。因为超时会打断一个循环——超时 → 兜底 → 本 tick 没产生计划 → 下个 tick 必须重新规划 → 更多调用。成功的规划会产出可跨 tick 复用的计划。代价落在 P95（tick 20.6s → 30.3s），P50 几乎不变。

这就是先补观测、再谈优化的回报：没有归因数据时，"该不该调超时"只能靠猜。默认值现已改为 30 秒（`Settings` 与 `.env.example` 同步，有防漂移测试）。

两次探路**不是受控 A/B**（相隔约一小时、外部服务不可控、样本都太小），哪些结论站得住、哪些只是信号，[快照](docs/eval/2026-09-16-stage3m-snapshot.md)里逐条标了。

顺带一个直观对照：那 5 个 tick 的墙钟耗时是 20.2 / 20.2 / 18.2 / **0.3** / 11.9 秒——第 4 个 tick 三个 NPC 全部复用已有计划、零模型调用，所以只花了 0.3 秒。

当前 HEAD 的 [20-tick Live 正式验收](docs/eval/2026-09-17-agent-eval-live-20tick.md)已通过：**动作合法率 100.0%（43/43）**，满足 spec §17.5 的 ≥90%，样本大于 2026-09-14 那次的 41 条且零规则拒绝。行为熵 2.126/2.585 为历次最高，6 个动词用了 5 个；计划复用 51.7%，其中 4 个 tick 因三个 NPC 全部复用而耗时不到 1 秒。

这次验收还给出了 taxonomy 最直接的回报——**两类失败的耗时特征完全相反**：
...
旧的合并实现下，这 3 次会被算进"超时"，读数的人会继续去调超时上限——但它们是服务端主动拒绝，再等多久都不会成功。两类需要完全不同的应对。

仍然偏低的是可用决策率 41.4%（12/29）。它不影响 §17.5——合法率衡量"拿到结果之后对不对"，可用率衡量"拿没拿到"，是并列的两个分母。瓶颈目前在服务端，不在提示词或动作契约。

[2026-09-14 的 20-tick Live 报告](docs/eval/2026-09-14-agent-eval.md)是**历史版本**；随后已修改并行规划、请求载荷和计划长度，不能把它与上面几份当成受控 A/B。旧报告称 19 次失败为超时，是当时的推断而非 trace 可复核的计数。
...
这里描述**开发者如何使用 AI 辅助工程工作**，与上面的 NPC Agent 运行链路是两件事。AI 帮助提出方案、写代码和排错；目标、范围、技术取舍、最终 review 与 Git 提交由人负责。
...
- 当前只有一张室外地图和一条主线任务。
- 真实模型评测和公网交互体验需要单独验证，不能用 Fake 报告代替。
