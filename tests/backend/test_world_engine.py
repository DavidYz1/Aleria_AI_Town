...
LOCATIONS = (
    LocationSnapshot(id="tavern", name="星辰酒馆", sort_order=1),
...
def test_initial_morning_snapshot_produces_expected_actions():
    snapshot = world(
        ...
        npc("grey", "Guardian", "park", 3, energy=86, mood=73, social=52),
        time="09:00",
    )
...
def test_role_routines_change_with_phase_and_location():
    ...
    assert decide_action(assassin, snapshot).target_id == "tavern"
    assert decide_action(guardian, snapshot).target_id == "park"
