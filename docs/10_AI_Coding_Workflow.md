...
本项目使用 AI 辅助开发的方式，一句话概括：

> 不让 AI 一次生成完整项目，而是先通过 Context Engineering 建立项目上下文，包括需求、架构、数据模型和 Prompt 设计；之后把任务拆解成独立模块交给 AI 实现，每次生成代码后进行人工 Review 和测试验证，确保 AI 输出符合整体架构。
