# Aleria AI Town 架构决策记录（Architecture Decision Record）

版本：v1.6

更新时间：2026-08-25

# 1. 文档目的

本文件用于记录 Aleria AI Town 项目中的关键技术与架构决策。

目标：

-   记录为什么选择某种方案
-   保留设计演进过程
-   方便后续开发维护
-   支持对外的技术方案解释

本项目不仅关注功能实现，同时关注：

> 为什么这样设计，以及这种设计带来的工程权衡。

------------------------------------------------------------------------

# 2. 架构决策原则

Aleria AI Town 遵循以下原则：

## 2.1 MVP优先

项目目标：

在有限时间内完成：

-   可运行
-   可演示
-   可解释

因此避免过早引入复杂基础设施。

------------------------------------------------------------------------

## 2.2 世界模型优先

系统设计围绕：

World State

而不是：

单次请求。

核心流程：

    World State

    ↓

    NPC Agent

    ↓

    Action

    ↓

    Event

    ↓

    World Update

------------------------------------------------------------------------

## 2.3 AI增强而非AI替代

LLM不直接控制整个系统。

采用：

    规则约束

    +

    LLM生成

保证：

-   可控
-   可解释
-   可测试

------------------------------------------------------------------------

# 3. ADR-001：整体系统架构选择

## 背景

项目需要支持：

-   Web前端
-   后端API
-   世界模拟
-   NPC Agent
-   数据存储

考虑方案：

## 微服务架构

优点：

-   高扩展性

缺点：

-   部署复杂
-   开发成本高
-   不适合2-3天MVP

## 简单脚本架构

优点：

-   开发快

缺点：

-   模块耦合
-   后续扩展困难

------------------------------------------------------------------------

## 决策

采用：

> 模块化单体架构（Modular Monolith）

结构：

    Backend

    ├── API Layer

    ├── World Engine

    ├── Agent System

    ├── Database Layer

    └── LLM Provider

------------------------------------------------------------------------

## 原因

适合：

-   AI应用快速迭代
-   小团队开发
-   后续模块拆分

------------------------------------------------------------------------

# 4. ADR-002：前端技术选型

## 背景

需要实现：

-   小镇展示
-   NPC状态展示
-   玩家交互

候选方案：

## 原生HTML + JavaScript

优点：

-   简单

缺点：

-   组件管理能力弱

------------------------------------------------------------------------

## React

优点：

-   生态成熟

缺点：

-   国内游戏业务团队不一定优先

------------------------------------------------------------------------

## Vue3 + TypeScript

最终选择。

原因：

-   项目需求明确支持
-   国内生态成熟
-   组件化开发效率高
-   类型系统提高代码质量

------------------------------------------------------------------------

# 5. ADR-003：后端技术选型

## 候选方案

Node.js

Go

Python

------------------------------------------------------------------------

## 决策

采用：

> Python + FastAPI

------------------------------------------------------------------------

## 原因

1.  已有项目经验

2.  AI生态完善

3.  Pydantic方便数据校验

4.  Swagger自动生成接口文档

------------------------------------------------------------------------

# 6. ADR-004：数据库选择

## 候选方案

## Redis

问题：

主要用于缓存。

不适合作为长期世界状态存储。

------------------------------------------------------------------------

## MySQL/PostgreSQL

优点：

生产级。

缺点：

MVP部署成本更高。

------------------------------------------------------------------------

## SQLite

最终选择。

原因：

-   数据量小
-   单机运行
-   部署简单
-   满足Demo需求

同时：

通过Repository Layer隔离数据库。

未来可以迁移：

    SQLite

    ↓

    PostgreSQL/MySQL

------------------------------------------------------------------------

# 7. ADR-005：NPC Agent架构选择

这是项目核心决策。

------------------------------------------------------------------------

## 方案一：纯规则系统

例如：

    if energy < 20:

        sleep

优点：

稳定。

缺点：

NPC行为固定，没有人格。

------------------------------------------------------------------------

## 方案二：纯LLM Agent

优点：

自由度高。

缺点：

-   行为不可控
-   可能违反世界规则
-   难测试

------------------------------------------------------------------------

## 决策

采用：

> Hybrid Agent（混合Agent）

架构：

    Rule Engine

    ↓

    Candidate Actions

    ↓

    LLM Decision

    ↓

    Action Validation

    ↓

    World Update

------------------------------------------------------------------------

## 原因

规则负责：

-   世界约束
-   合法性

LLM负责：

-   人格体现
-   行为解释
-   对话生成

------------------------------------------------------------------------

# 8. ADR-006：Player系统设计

## 候选方案

## 不设计玩家系统

问题：

玩家无法真正参与世界。

------------------------------------------------------------------------

## 完整RPG系统

包括：

-   等级
-   装备
-   战斗

问题：

范围失控。

------------------------------------------------------------------------

## 决策

采用：

> 轻量玩家档案

Player也是：

World Entity。

拥有：

-   名字
-   职业
-   位置
-   状态
-   关系
-   记忆

------------------------------------------------------------------------

## 原因

统一：

NPC

Player

都是世界参与者。

方便未来扩展：

-   Quest
-   Relationship
-   Event

------------------------------------------------------------------------

# 9. ADR-007：地图实现方案

## 候选方案

## 一开始使用Canvas/Cocos

优点：

游戏感强。

缺点：

开发成本高。

------------------------------------------------------------------------

## 决策

第一阶段：

Vue组件 + CSS布局。

未来：

Canvas/PixiJS/Cocos。

------------------------------------------------------------------------

## 原因

保持：

World Model

和：

Rendering Layer

解耦。

------------------------------------------------------------------------

# 10. ADR-008：Memory设计

## 候选方案

直接引入：

-   RAG
-   Vector Database

问题：

复杂度较高。

------------------------------------------------------------------------

## 决策

MVP：

SQLite Memory表。

保存：

-   重要事件
-   NPC经历

未来：

增加：

Embedding

↓

Vector Retrieval

------------------------------------------------------------------------

# 11. ADR-009：LLM Provider设计

## 问题

如果业务代码直接调用某个模型：

会导致：

-   模型绑定
-   难测试

------------------------------------------------------------------------

## 决策

设计统一接口：

    ChatProvider
         |
         |---- MockChatProvider
         |
         └---- FallbackChatProvider
                   ├── OpenAICompatibleChatProvider
                   └── MockChatProvider

Phase 1C 不为腾讯混元、DeepSeek、本地 Qwen 分别复制 Provider 类。它们通过 `base_url/api_key/model/auth_mode` 复用一个 compatible Adapter；未来非 compatible 协议再新增独立 Adapter。

------------------------------------------------------------------------

## 原因

支持：

-   多模型切换
-   无密钥运行
-   自动测试

------------------------------------------------------------------------

# 12. ADR-010：开发策略

## 问题

容易陷入：

同时开发：

-   AI
-   地图
-   任务
-   动画

导致项目无法完成。

------------------------------------------------------------------------

## 决策

采用：

MVP → Enhancement

阶段：

## Phase 1

满足腾讯基础要求：

-   小镇页面
-   NPC展示
-   Tick推进
-   对话

------------------------------------------------------------------------

## Phase 2

AI增强：

-   Memory
-   Relationship
-   更强Agent

------------------------------------------------------------------------

## Phase 3

游戏体验：

-   Canvas地图
-   动画
-   Quest

------------------------------------------------------------------------

# 13. ADR-011：Context Engineering驱动开发

## 背景

AI辅助开发最大问题：

不是生成代码能力。

而是：

上下文管理。

------------------------------------------------------------------------

## 决策

采用：

文档驱动开发。

结构：

    docs/

    00_Project_Context

    01_Assignment

    02_Product

    03_World_Model

    04_Agent

    05_Architecture

    06_API

    07_Database

    08_Prompt

    09_Decision_Log

------------------------------------------------------------------------

## 开发流程

    人工设计

    ↓

    Context文档

    ↓

    AI理解项目

    ↓

    任务拆解

    ↓

    代码生成

    ↓

    人工Review

    ↓

    测试验证

------------------------------------------------------------------------

## 价值

降低：

-   AI理解偏差
-   上下文丢失
-   需求漂移

------------------------------------------------------------------------

# 14. 后续决策

未来可能继续记录：

-   Canvas方案
-   Quest系统
-   多Agent协作
-   部署方案
-   性能优化

------------------------------------------------------------------------

# 15. ADR-012：Phase 0即使用SQLite

## 背景

Phase 0可以直接从JSON或内存返回世界数据，也可以从第一条API开始建立持久化边界。

## 决策

采用：

> SQLite作为运行时唯一状态源，根目录JSON只作为可读、可版本管理的种子输入。

Phase 0只创建 `world_state`、`locations`、`npc_profiles`、`npc_states` 四组表。

## 原因

-   尽早验证Repository边界。
-   为World Tick提供真实状态基础。
-   保持初始内容可读，同时避免JSON与SQLite形成两个运行时可信源。
-   控制物理表数量，不提前实现尚无用例支撑的Memory、Relationship和Snapshot。

------------------------------------------------------------------------

# 16. ADR-013：统一公共契约命名

## 决策

-   世界回合统一使用 `tick`。
-   NPC职业统一使用 `role`。
-   NPC当前位置统一使用 `location_id`。
-   NPC需求 `energy`、`mood`、`social` 统一为0-100整数。
-   World、Location和NPC统一使用稳定的小写字符串ID。
-   第一版Action集合统一为 `move`、`rest`、`work`、`eat`、`social`。

## 原因

避免API、数据库、Prompt、Validator和Frontend分别形成不兼容字段。

------------------------------------------------------------------------

# 17. ADR-014：开源前端迁移限定在展示层

## 决策

Frontend保持：

    Typed API Adapter
    ↓
    World Store
    ↓
    UI / Renderer

未来引入开源Vue界面、PixiJS或其他2D渲染方案时，只替换组件、样式、素材和Renderer，不以模板数据结构反向修改World API和Backend领域模型。

## 原因

-   保证Backend继续作为世界唯一状态源。
-   降低前端视觉迁移对Agent和数据层的影响。
-   可以独立审查开源许可证、素材授权和依赖风险。

------------------------------------------------------------------------

# 18. ADR-015：Phase 1A使用确定性同快照决策

## 决策

所有NPC在时钟推进和统一需求衰减后，从同一个不可变快照选择Action。决策只依赖当前状态、角色特点、morning/day/evening/night阶段与稳定 `sort_order`。

## 原因

-   同输入必然产生同输出，便于测试、演示和排错。
-   避免NPC数据库顺序或当前Tick执行顺序影响结果。
-   先验证世界规则闭环，后续Hybrid Agent只需替换Decision Policy。

------------------------------------------------------------------------

# 19. ADR-016：Tick使用expected_tick乐观锁与单事务

## 决策

Frontend提交当前 `expected_tick`；Backend通过条件更新保证同一版本只有一个请求成功。世界/NPC当前状态与Action/Event历史在一个事务中提交。

## 原因

防止重复点击或并发请求造成时间跳跃、部分NPC更新或重复历史。

------------------------------------------------------------------------

# 20. ADR-017：Tick返回完整World而非Patch

## 决策

成功响应返回完整 `WorldData + actions + events`。409时Frontend重新GET权威世界。

## 原因

保持Frontend Store简单，避免客户端自行合并状态导致契约漂移，也为后续替换UI/Renderer保留稳定边界。

------------------------------------------------------------------------

# 21. ADR-018：NPC Detail使用独立只读切片与确定性解释

## 背景

Phase 1B 需要展示 NPC 的权威当前状态和最近行动，同时不能破坏已经验证的确定性 Tick 写入路径，也不应把规则解释误表述为模型思维过程。

## 决策

-   新建 `NpcRepository → NpcService → GET /api/npcs/{npc_id}` 只读切片，不向 `WorldTickRepository` 添加详情查询。
-   复用 `npc_profiles`、`npc_states`、`locations`、`world_state`、`actions`，不新增数据库表。
-   数据库保留稳定 `actions.reason` 机器代码；Detail DTO 对外提供 `reason_code` 和根据历史上下文确定生成的 `reason_text`。
-   解释是可审计的规则摘要，不重算历史决策，不保存或暴露 chain-of-thought/Agent Trace。
-   Frontend 使用独立 `npcDetail` Store 处理选择、竞态、重试和刷新；`TownView` 协调 World/NPC Detail Store，两个 Store 不相互依赖。

## 原因

-   保护 Tick 的单事务写入边界，让详情查询可独立演进。
-   保持 Backend 作为状态与历史的唯一事实来源。
-   在可解释、可测试和不暴露隐藏推理之间保持清晰边界。

------------------------------------------------------------------------

# 22. ADR-019：真实模型使用单一 OpenAI-compatible Adapter

## 决策

`ChatService` 只依赖 `ChatProvider`。除 Mock 外的 Provider 标签统一由 `OpenAICompatibleChatProvider` 处理，配置为 `base_url/api_key/model/auth_mode/timeout`，不创建 Hunyuan、DeepSeek 或 Qwen 专用 Service 分支。

## 原因

-   避免供应商类重复和配置漂移。
-   同时支持云端 API 与本地 compatible 服务。
-   保留未来新增 Gemini native 等非 compatible Adapter 的能力。

------------------------------------------------------------------------

# 23. ADR-020：Mock 是一等运行模式与自动 fallback

## 决策

默认 `CHAT_PROVIDER=mock`，无 URL、model、Key 也能完成验收闭环。非 Mock Primary 的可预期 Provider 故障由 `FallbackChatProvider` 降级到真实 `MockChatProvider`，并如实返回和保存 `provider=mock`、`fallback_used=true`。

## 原因

-   项目可以在无外网、无密钥环境稳定演示和测试。
-   fallback 不伪装成 Primary 成功，便于诊断和 UI 提示。
-   仅捕获 `ChatProviderError`，不吞掉编程错误。

------------------------------------------------------------------------

# 24. ADR-021：Chat 与确定性 World Engine 隔离

## 决策

Chat Context 可以读取 World、NPC State 和最近 Action，但 Chat 路径不得调用 World Tick、更新 NPC State 或插入 Action/Event。聊天历史只进入后续 Chat Context，不影响 Phase 1A 的确定性 Action Policy。

## 原因

-   保持同输入世界快照产生同一 Action 的可重复性。
-   防止自然语言输出绕过 Action Validation 和事务边界。
-   未来 Memory/Relationship 进入世界决策前必须另行设计并审查。

------------------------------------------------------------------------

# 25. ADR-022：只原子保存完整聊天轮次

## 决策

Provider 调用发生在数据库写事务之外。只有回复通过严格 `reply + emotion` 校验后，Repository 才在一次事务中创建/更新 Conversation，并同时插入 User 与 Assistant；任一持久化错误整体回滚。

## 原因

-   Provider 或数据库故障不留下只有 User 的半轮记录。
-   Assistant 行可以保存实际 Provider、fallback 和 Prompt 版本，支持审计。
-   有界历史始终由完整的持久化消息构成。

------------------------------------------------------------------------

# 26. ADR-023：世界显示名演进但稳定 ID 不变

## 决策

世界显示名改为“曦谷”，酒馆改为“星辉酒馆”；保留 `aleria-town/tavern/park`，并新增 `castle/forest`。API、数据库关系和前端状态始终使用稳定 ID。

## 原因

产品文案可以继续演进，但不应因此触发数据库主键迁移或破坏已有客户端。

------------------------------------------------------------------------

# 27. ADR-024：首个任务使用专用确定性状态机

## 决策

只实现 `missing-child`，由纯 Policy 定义五次迁移；不提前建设 Quest DSL、奖励、背包或通用条件系统。Progress 使用 version 乐观锁，Progress 与 Event 同事务保存。

## 原因

一个任务不足以证明通用抽象，专用 Policy 更易测试，也能完整展示持久化游戏闭环。

------------------------------------------------------------------------

# 28. ADR-025：任务 UI 先采用 DOM-first

## 决策

Phase 1D 在 Vue DOM 页面完成旅行和任务交互，Frontend 只展示 Backend 返回的 objective/available interactions，不复制状态机。PixiJS/Cocos 作为后续展示层迁移。

## 原因

先稳定 API 和 Store，可降低地图资源、碰撞与动画对核心验收的干扰，未来替换渲染层也无需重写业务接口。

------------------------------------------------------------------------

# 29. ADR-026：compatible Adapter 支持结构化与文本双模式

## 决策

保留单一 `OpenAICompatibleChatProvider`，通过 `CHAT_LLM_OUTPUT_MODE=structured_json|text` 适配模型输出能力。Text 模式仍校验回复，并确定性派生公共 emotion。

## 原因

`hy-role` 返回自然文本时，复制供应商专用 Provider 会造成重复；输出适配属于协议 Adapter 职责，不应污染 ChatService、Fallback 或 API。

------------------------------------------------------------------------

# 30. ADR-027：Grey 任务交互使用实时共址校验

## 决策

`ask_grey` 只有在玩家与 Grey 的权威 `location_id` 相同时可执行，objective 动态展示 Grey 当前地点。旅行仍不推进 Tick，Quest 暂不干预 NPC 行为。

## 原因

这修复了“玩家到城堡但 Grey 已离开仍能询问”的世界一致性问题，同时避免引入任务驱动 NPC 调度或隐藏时间推进。

------------------------------------------------------------------------



# 31. ADR-028：Story Bible 作为内容事实源

## 决策

docs/15_Story_Bible_CN.md 是世界时间线、地点名称、角色经历和信息分层的唯一内容事实源。其他产品、Prompt 和提交文档只引用适合自身受众的层级。

## 原因

集中事实源可以避免终焉战争、灰烬战争、Grey 经历与地点名称在多份文档中漂移。

------------------------------------------------------------------------

# 32. ADR-029：Author Truth 不进入运行时 Prompt

## 决策

完整 Author Truth 只服务于作者规划，不整体注入 World Lore、Character Prompt 或 README。运行时只组合 Public Lore、Player Context、角色自己的 Character Knowledge 和 Backend 权威状态。

## 原因

限制信息可见性可以防止 NPC 全知泄密，并让调查、误解和角色矛盾成为玩法的一部分。

------------------------------------------------------------------------

# 33. ADR-030：Prompt v3 按角色划分知识边界

## 决策

Prompt v3 为 Ryan、Shir、Grey 分别记录确认知道、怀疑、错误相信、隐瞒和禁止事项；v1/v2 保留为显式回退资产。

## 原因

角色差异来自立场与知识，而不只是语气词。版本化资产允许内容升级，同时保持配置回退和自动测试能力。

------------------------------------------------------------------------

# 34. ADR-031：hy-role 是场景化实测推荐

## 决策

基于本项目 NPC 对话体验，推荐 hy-role + text mode 作为首选真实模型；该结论只适用于本项目场景，不构成通用排名，也不形成 Hunyuan 专用业务分支。

## 原因

hy-role 的自然角色表达较好，但不稳定遵守 JSON 输出契约。让统一 OpenAI-compatible Adapter 处理 text mode，既保留体验优势，也保持 ChatService、Fallback 和公共 API 的供应商无关性。

------------------------------------------------------------------------

# 35. ADR-032：Phase 2 选择 Phaser 3.90.0 而不是 PixiJS

## 决策

RPG 展示层使用固定版本 Phaser 3.90.0。PixiJS 仍是优秀的轻量渲染器，但本阶段需要 Tilemap、Arcade Physics、碰撞、相机、键盘输入、Sprite 动画和 Scene 生命周期；Phaser 已提供这些游戏语义。

## 原因

选择 Phaser 可以把有限时间用于业务边界和可玩闭环，而不是在 PixiJS 上自行组装地图解析、碰撞和生命周期。代价是 bundle 更大，因此通过 `TownGameHost` 动态加载，并保持纯逻辑投影函数不依赖 Phaser。

------------------------------------------------------------------------

# 36. ADR-033：Vue DOM 与 Phaser Canvas 分工

## 决策

Vue/Pinia 继续负责启动、角色创建、剧情、World Tick、Quest、语义 travel、NPC Detail/Chat、错误和可访问替代入口；Phaser 只负责单地图、角色移动、碰撞、相机、动画和 NPC 点击命中。Canvas 事件通过 `TownGameBridge` 转成稳定 ID，再由 TownView 调用现有 Store。

## 原因

这样地图加载失败不会隐藏任务和居民卡片，也避免 Phaser 直接调用 API 或复制 Vue 业务状态。渲染层可以替换，Backend 权威和现有测试闭环不受影响。

------------------------------------------------------------------------

# 37. ADR-034：玩家采用双层位置模型

## 决策

Backend `location_id` 是 Player、NPC 和 Quest 的权威语义位置；Phaser `(x, y)` 只是当前 Canvas 会话的表现状态，不持久化、不进入 API payload，也不会由 WASD 触发 `/api/player/travel`。语义 travel 仍只由 DOM LocationCard 发起。

## 原因

像素移动和任务地点具有不同精度与生命周期。分层可防止刷新、碰撞或客户端篡改破坏任务状态，也避免为演示层扩充 Player Schema 和 migration。

------------------------------------------------------------------------

# 38. ADR-035：Phase 2 只接收可追溯 CC0 素材

## 决策

地图、角色和提示音只使用 `THIRD_PARTY_ASSETS.md` 登记的 CC0 来源；参考仓库只用于架构和交互研究，不复制其二进制素材。源图允许裁切、最近邻缩放和透明色键转换，但必须记录转换。

## 原因

许可清晰、来源可复核，比临时混用来源不明的美术更适合公开交付，也让后续替换视觉资产不会触碰业务代码。

------------------------------------------------------------------------
# 39. ADR-036：确定性、同步的可观测 Agent Runtime（2026-09-07）

本条与以下新决策覆盖前文早期阶段的未来架构设想；历史 ADR 保留其当时语境。

本阶段采用 Snapshot -> ActionProposal -> Registry validation -> deterministic resolution -> atomic world/run/action/event/trace commit。每个 NPC 使用相同不可变决策快照，Registry 是合法行为、目标和效果的唯一权威。初始行为为 move/rest/work/eat/talk/wait；social 仅保留为 NPC 需求数值。

POST /api/world/tick 继续同步返回 200；RPG 保持一个“推进 1 小时”入口。GET /api/agent-runs/{run_id} 提供有序 proposal/event/trace，摘要由结构化事实生成，不持久化隐藏推理、不返回原始 Prompt 或密钥。异步 202、队列、Agent Lab 模式控制需要后续完整生产运行时设计，本阶段不引入。

# 40. ADR-037：区分世界版本、时钟与事件序号

world_version 是所有权威变更共享的乐观并发 token；实际旅行、任务迁移和世界推进各增加一次。clock_tick 仅随游戏时间推进增加。event_sequence 在同一世界内严格连续排序，与状态在同一事务提交。任务还保留自身 expected_version，普通聊天不改变世界版本或时钟。

# 41. ADR-038：Alembic + 本地 SQLite / Docker PostgreSQL 双模式

Alembic 0001–0003 负责空库和受支持旧 SQLite 数据库升级。旧行为历史迁移为可观察运行记录；正常启动升级 schema 并仅在缺少世界时播种，不做无条件 reset。

本地 .env.example 和快速测试继续使用 SQLite。Docker 默认 PostgreSQL 17，镜像固定 pgvector/pgvector:0.8.6-pg17-bookworm，驱动为 Psycopg 3；数据库健康后 Backend 才启动，Backend 健康后 Web 才启动。数据库持久卷为 aleria_postgres_data；base Compose 不发布数据库端口，测试 override 仅绑定 loopback。旧 SQLite 卷保留，跨数据库搬迁不在本阶段自动执行。

pgvector 扩展已由迁移启用，但没有 vector ORM 列；首个向量列属于后续 Memory 阶段。PostgreSQL 测试仅由 TEST_POSTGRES_URL 显式启用，在各自空 schema 中验证从零迁移及完整运行。缺少 URL/Compose/daemon 时如实记录 skip/unavailable，不视为真实冒烟通过。

# End of Document
